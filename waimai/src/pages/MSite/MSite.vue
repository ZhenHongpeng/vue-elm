...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      // 根据categorys一维数组生成一个categoryArr二维数组(元素个数最大是8个)
      categoryArr(){
        ...
        categorys.forEach(c => {
          ...
          // 如果minArr的长度达到8,就创建一个新的minArr
          if (minArr.length ===8 ){
            minArr = []
          }

          // 如果minArr是空的,就把小数组保存到大数组中
          if(minArr.length===0) {
            arr.push(minArr)
          }
    ...
    watch:{
      categorys (value){ //categorys 数组中有数据了,在异步更新界面之前执行
        ...
        // setTimeout(()=>{ // 可以实现,但不推荐
        //   // 创建一个Swiper实例对象,来实现轮播,要在列表显示之后
        ...
        // 界面更新就立即创建Swiper对象
        this.$nextTick(()=>{  // 一旦界面更新立即创建Swiper对象
          new Swiper('.swiper-container',{
                direction:'horizontal',
                loop:true,
                // 分页器
